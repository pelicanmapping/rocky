...
namespace rocky { namespace util
{
    ...
    template<typename T>
    struct PerThread : public util::Mutex
    {
        PerThread() : util::Mutex() { }

        PerThread(const std::string& name) : util::Mutex(name) { }

        T& get() {
            util::ScopedMutexLock lock(*this);
    ...
    struct PerObjectMap
    {
        ...
        {
            {
                rocky::util::ScopedReadLock readLock(_mutex);
                typename std::unordered_map<KEY,DATA>::iterator i = _data.find(k);
                if ( i != _data.end() )
                    return i->second;
            }
            {
                rocky::util::ScopedWriteLock lock(_mutex);
                typename std::unordered_map<KEY,DATA>::iterator i = _data.find(k);
                if ( i != _data.end() )
                    return i->second;
                else
        ...
        void remove(KEY k)
        {
            rocky::util::ScopedWriteLock exclusive(_mutex);
            _data.erase( k );
        }

    private:
        std::unordered_map<KEY,DATA> _data;
        rocky::util::ReadWriteMutex _mutex;
    };
    ...
    struct PerObjectFastMap
    {
        ...
        DATA& get(KEY k)
        {
            rocky::util::ScopedMutexLock lock(_mutex);
            typename std::unordered_map<KEY,DATA>::iterator i = _data.find(k);
            if ( i != _data.end() )
                return i->second;
            else
    ...
    private:
        std::unordered_map<KEY,DATA> _data;
        mutable rocky::util::Mutex _mutex;
    };
    ...
    struct PerObjectRefMap
    {
        ...
        DATA* get(KEY k)
        {
            rocky::util::ScopedReadLock lock(_mutex);
            typename std::unordered_map<KEY,std::shared_ptr<DATA>>::const_iterator i = _data.find(k);
            if ( i != _data.end() )
                return i->second.get();
        ...
        DATA* getOrCreate(KEY k, DATA* newDataIfNeeded)
        {
            ...
            {
                rocky::util::ScopedWriteLock lock(_mutex);
                typename std::unordered_map<KEY, std::shared_ptr<DATA>>::iterator i = _data.find(k);
                if ( i != _data.end() )
                    return i->second.get();
        ...
        {
            rocky::util::ScopedWriteLock exclusive(_mutex);
            for( typename std::unordered_map<KEY, std::shared_ptr<DATA>>::iterator i = _data.begin(); i != _data.end(); ++i )
            {
    ...
    private:
        std::unordered_map<KEY,std::shared_ptr<DATA>> _data;
        rocky::util::ReadWriteMutex _mutex;
    };
    ...
    public:
        ...
        void push(const T& t) {
            rocky::util::ScopedMutexLock lock(_m);
            _queue.push(t);
        }
