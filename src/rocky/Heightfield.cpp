...
float
Heightfield::heightAtUV(double nx, double ny, Interpolation interpolation) const
{
    double px = clamp(nx, 0.0, 1.0) * (double)(width() - 1);
    double py = clamp(ny, 0.0, 1.0) * (double)(height() - 1);
    return heightAtPixel(px, py, interpolation);
}

float
Heightfield::heightAtPixel(double c, double r, Interpolation interpolation) const
{
    float result = 0.0;
    ...
    case Interpolation::Triangulate:
    {
