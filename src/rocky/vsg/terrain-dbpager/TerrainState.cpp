...
TerrainState::inheritTerrainTileDescriptors(
    const TerrainTileRenderModel& input,
    const glm::dmat4& scaleBias,
    vsg::ref_ptr<vsg::StateGroup> stategroup,
    Runtime& runtime) const
{
    TerrainTileRenderModel model = input;

    if (model.color.image)
        model.color.matrix *= scaleBias;
    if (model.elevation.image)
        model.elevation.matrix *= scaleBias;
    if (model.normal.image)
        model.normal.matrix *= scaleBias;
    if (model.colorParent.image)
        model.colorParent.matrix *= scaleBias;

    TerrainTileDescriptors::Uniforms uniforms;
    uniforms.color_matrix = model.color.matrix;
    uniforms.elevation_matrix = model.elevation.matrix;
    uniforms.normal_matrix = model.normal.matrix;

    vsg::ref_ptr<vsg::ubyteArray> data = vsg::ubyteArray::create(sizeof(uniforms));
    memcpy(data->dataPointer(), &uniforms, sizeof(uniforms));
    model.descriptors.uniforms = vsg::DescriptorBuffer::create(data, TILE_BUFFER_BINDING);

    auto descriptorSetLayout = pipelineConfig->layout->setLayouts.front();

    auto descriptorSet = vsg::DescriptorSet::create(
        descriptorSetLayout,
        vsg::Descriptors{ model.descriptors.elevation, model.descriptors.color, model.descriptors.normal, model.descriptors.uniforms }
    );
