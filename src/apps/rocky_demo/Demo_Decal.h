...
auto Demo_Decal_Orthographic = [](Application& app)
{
    ...
    if (e_decal == entt::null)
    {
        ...
        xform.localMatrix = glm::scale(glm::dmat4(1), glm::dvec3(scale)); // 10km wide
...
auto Demo_Decal_Perspective = [](Application& app)
{
    ...
    if (ImGuiLTable::Begin("decal-persp"))
    {
        ...
        auto& [optics, style] = reg.get<Optics, DecalStyle>(e_decal);
        ...
        if (ImGuiLTable::SliderDouble("Pitch", &xaxis, 10.0, 90.0, "%.1lf"))
        {
            optics.pose = glm::mat4_cast(quaternion_from_euler_degrees(xaxis, yaxis, zaxis));
        }

        ImGuiLTable::SliderDouble("Near scale", &optics.nearScale, 0.0, 1.0, "%.2lf");
        ImGuiLTable::SliderDouble("Near bias", &optics.nearBias, -100000.0, -100.0, "%.1lf");
        ImGuiLTable::SliderDouble("Far scale", &optics.farScale, 1.0, 2.0, "%.2lf");
        ImGuiLTable::SliderDouble("Far bias", &optics.farBias, 100.0, 100000.0, "%.1lf");
...
auto Demo_Decal_Projector = [](Application& app)
{
    ...
    static entt::entity e_frustum = entt::null;
    static CallbackSubs subs;
    const std::string textureFile = "https://readymap.org/readymap/filemanager/download/public/textures/reference_grid_Color.jpg";
    const std::string modelFile = "https://readymap.org/readymap/filemanager/download/public/models/C130_WFF_AIR_0824.glb";

    if (e_platform == entt::null)
    {
        // load up a texture image
        auto& io = app.io();
        auto r = io.services().readImageFromURI(textureFile, io);
        Image::Ptr image;
        if (r.ok()) {
            image = r.value();
        }
        ...
        e_frustum = reg.create();
        auto& frustum_transform = reg.emplace<Transform>(e_frustum);
        auto& lineStyle = reg.emplace<LineStyle>(e_frustum);
        lineStyle.color = StockColor::White;
        auto& lineGeom = reg.emplace<LineGeometry>(e_frustum);
        lineGeom.topology = LineTopology::Segments;
        lineGeom.points.resize(24);
        auto& line = reg.emplace<Line>(e_frustum, lineGeom, lineStyle);
        ...
        auto animate = [&app, motionsys, heading, pitch](VSGContext) mutable
        {
            ...
            auto& frustum_xform = r.get<Transform>(e_frustum);
            auto& lineGeom = r.get<LineGeometry>(e_frustum);

            frustum_xform.position = platform_xform.position;
            frustum_xform.topocentric = platform_xform.topocentric;
            frustum_xform.localMatrix = platform_xform.localMatrix * optics.pose;
